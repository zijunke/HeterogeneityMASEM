...
opts_knit$set(root.dir = 'D:/Research/2023/CompareMASEM/CFA/Bifactor/WithCovariate/')
```
...
# Working directory
wd = 'D:/Research/2023/CompareMASEM/CFA/Bifactor/WithCovariate/'
setwd(wd)
```
...
data<-list("Nstudy","N","mu.N",'p',"pp","j","k",'j10','k10','ind',
   "vR","tau.vR",'M',"mu.vR.psi",'alpha.prior.vE','beta.prior.vE') 

#Initial values
vR.inits = vR;vR.inits[which(is.na(vR)==0,arr.ind = TRUE)] = NA
vL.inits = matrix(0.6,Nstudy,p*2);vL.inits[,15] = NA
initsl <- list(list(a=rep(0.6,p*2),b=c(rep(0,14),NA,rep(0,5)),
   sd.uL = c(rep(0.1,14),NA,rep(0.1,5)),tau.R=mu.N*3,
   vR.psi = matrix(0,Nstudy,pp),vR = vR.inits,vR.rep = vR,vL = vL.inits))

prm =c('a','b','sd.uL','vLH.pred','vLL.pred') # Parameters to save
...
M1 <- create.vechsR(A0=RAM0$A, S0=RAM0$S, F0=RAM0$F, Ax=list(Ax1))

## Fit the bifactor model with One-Stage MASEM
fit1 <- osmasem(model.name="Moderated by individualism", Mmatrix=M1, 
